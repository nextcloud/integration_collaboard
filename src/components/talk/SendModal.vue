<template>
	<NcModal
		...
		<div class="modal-content">
			<h2>
				{{ t('integration_collaboard', 'Send link to a Talk room') }}
			</h2>
			<div class="input-wrapper">
				<input ref="query"
					v-model="query"
					type="text"
					:placeholder="t('integration_collaboard', 'Search for users, groups or conversations')"
			...
			<div class="results">
				<div v-show="conversationsToShow.length > 0" id="conversations">
					<h3>
						{{ t('integration_collaboard', 'Conversations') }}
					</h3>
					<ul>
				...
				<div v-show="usersToShow.length > 0" id="users">
					<h3>
						{{ t('integration_collaboard', 'Users') }}
					</h3>
					<ul>
				...
				<div v-show="groupsToShow.length > 0" id="groups">
					<h3>
						{{ t('integration_collaboard', 'Groups') }}
					</h3>
					<ul>
	...
	methods: {
		...
		createRoom() {
			...
			axios.post(url, req).then((response) => {
				...
				showSuccess(t('integration_collaboard', 'You created a conversation with {name}', { name: this.selectedRoom.displayName }))
		...
		joinOpenRoom() {
			...
			axios.post(url, req).then((response) => {
				...
				showSuccess(t('integration_collaboard', 'You joined {name}', { name: this.selectedRoom.displayName }))
				this.sendLink()
			}).catch((error) => {
				showError(
					t('integration_collaboard', 'Failed to join')
		...
		sendLink() {
			...
			const req = {
				message: t('integration_collaboard', 'Collaboard project "{name}": {link}', {
			...
			axios.post(url, req).then((response) => {
				console.debug('TALK response', response)
				showSuccess(t('integration_collaboard', 'Link sent to {name}', { name: this.selectedRoom.displayName }))
				this.$emit('close')
			}).catch((error) => {
				showError(
