	...
	<div class="creationForm">
			...
			<div v-for="(field, fieldId) in creationFields"
				...
				<div v-else-if="field.type === 'ncColor'">
					<NcColorPicker
						...
						<NcButton
							v-tooltip.top="{ content: t('integration_collaboard', 'Choose color') }"
		...
		<div class="form-footer">
			...
			<NcButton type="primary" @click="onOkClick">
				...
				{{ t('integration_collaboard', 'Create') }}
			</NcButton>
		</div>
	</div>
</template>
		...
		onOkClick() {
			...
			Object.keys(this.fields).forEach((fieldId) => {
				...
				if (field.mandatory && !fieldValue && fieldValue !== false) {
					showError(t('integration_collaboard', 'Field "{name}" is missing', { name: field.label }))
					isFormValid = false
				}
			})
			if (isFormValid) {
